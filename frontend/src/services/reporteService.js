...
class ReporteService {
    ...
    generarReporteOperacional(productos, categoria = 'Todas') {
        ...
        doc.autoTable({
            ...
            didParseCell: function(data) {
                // Si la columna es 'Stock' (índice 3) y el valor es 0
                if (data.column.index === 3 && data.cell.raw === 0) {
                    data.cell.styles.textColor = [225, 29, 72]; // Rose 600 (danger)
        ...
        doc.save(`reporte_inventario_${categoria.toLowerCase().replace(' ', '_')}_${Date.now()}.pdf`);
        return doc;
    }
    ...
    generarReporteGestion(kpis, topCategorias, productosBajoStock) {
        ...
        this._agregarPieDePagina(doc);
        doc.save(`analisis_gestion_${Date.now()}.pdf`);
        return doc;
    }
}
